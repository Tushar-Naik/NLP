...
//------------------------------ Actual Logic of Autocomplete ------------------------
    ...
    public boolean createCompletableList(final int position, final String subword)
    {
	...
	//-------------------- Add Words for Autocomplete ------------------------------------------
			...
			if(separator.contains(e.getKeyChar()))//e.getKeyChar() == ' ' || e.getKeyChar() == '.' || e.getKeyChar() == '\n' || e.getKeyChar() == ',' || e.getKeyChar() == ';')
			{
				...
				if(lastWord.length()>7&&!Completable.contains(lastWord) && obj.nWords.containsKey(lastWord))
